...
def highlight_differences(image1, image2, sensitivity_threshold=45, blur_value=(21, 21)):
    ...
    blur_value = (max(1, blur_value[0] // 2 * 2 + 1),
                  max(1, blur_value[1] // 2 * 2 + 1))
    ...
    # Present in image1 but not in image2
    diff1 = cv2.subtract(blurred1, blurred2)
    # Present in image2 but not in image1
    diff2 = cv2.subtract(blurred2, blurred1)

    # Apply a binary threshold to highlight significant differences
    _, thresh_diff1 = cv2.threshold(
        diff1, sensitivity_threshold, 255, cv2.THRESH_BINARY)
    _, thresh_diff2 = cv2.threshold(
